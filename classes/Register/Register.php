...
class RMP_Register_Register {
	...
	public static function php_version_check() {
		if ( version_compare( phpversion(), static::php_version_required(), '<' ) ) {
			add_action( 'admin_notices', array( 'RMP_Register_Register', 'unsupported_php_version' ), 10, 2 );
			return false;
		}
		return true;
	}
	...
	public static function unsupported_php_version() {
		$short = __( 'You are running an unsupported version of PHP.', 'rmp-restmenu' );
	...
	public static function unsupported_wp_version() {
		$short = __( 'You are running an unsupported version of WordPress.', 'rmp-restmenu' );
	...
	private static function display_admin_notice( $short, $long ) {
		...
		<div class="notice notice-error">
			<p style="max-width:800px;">
				<b><?php echo esc_html( sprintf( _x( '%s can not be activated.', 'Plugin title', 'rmp-restmenu' ), static::$title ) );?></b>
	...
	public static function check_dependency() {
		if ( current_user_can( 'manage_options' ) ) {
			if ( ! function_exists( self::$dep_func ) ) {
				...
				deactivate_plugins( RMP_BASE ); // FIXME:  plugin name
...
/*
	private static function dependency_string() {
		$site_name = _x( 'The Creative Collective', 'noun - plugin site name', 'rmp-restmenu' );
		$comp_name = _x( 'The Creative Collective', 'noun - plugin company name', 'rmp-restmenu');
		$string    = _x( 'This plugin should only be used with %1$s themes by %2$s', '1: company name, 2: website title', 'rmp-restmenu' );
...
}  #  End of class RMP_Register_Register
